...
class Main
{
	...
	static function createCppStub(classInfo:ClassInfo)
	{
		...
		var fileName = Std.string("${dirCppSrc}/${prefix}.cpp");
		...
		for (method in classInfo.methods)
		{
            ...
            else
            {
                ...
                for (index in 0...params.length)
                {
                    ...
                    stream.writeString(Std.string("value ${param.name}"));
			...
			stream.writeString(
                    if (numParams > 5)
                        Std.string("DEFINE_PRIM_MULT($name);\n")
                    else
                        Std.string("DEFINE_PRIM($name, ${numParams});\n")
                );
			stream.writeString("\n");
		}
		...
		trace(Std.string("INFO: C++ source file\"${fileName}\" written."));
	}
    ...
    static function writeCppBody(stream:Output, classInfo:ClassInfo, methodInfo:MethodInfo)
    {
        ...
        if (methodInfo.type == mtMember)
        {
            ...
            params.push(new ParamInfo(Std.string("$name *thisObj")));
        }
        ...
        if (params.length > 5)
        {
            ...
            {
                stream.writeString("    ");
                stream.writeString(Std.string("const value& ${param.name} = "));
                stream.writeString(
        ...
        while (params.length > 0)
        {
            ...
            if (param.isPrimitive)
            {
                ...
                {
                    stream.writeString(Std.string("$modifier"));
                    stream.writeString(" ");
                }
                stream.writeString(Std.string("${param.type}"));
                stream.writeString(" ");
                if (param.isPointer)
                    stream.writeString("*");
                stream.writeString(Std.string("_${param.name} = "));
                ...
                stream.writeString(Std.string("(${param.name});\n"));
            }
            else
            {
                ...
                {
                    stream.writeString(", *");
                    stream.writeString(Std.string("_${other.name}"));
                }
	...
	static function createHaxeStub(classInfo:ClassInfo)
	{
		...
		var fileName = Std.string("${dirHaxeSrc}/${className}.hx");
		//if (FileSystem.exists(fileName))
			//return;

		//var stream:Output = Sys.stdout();
		var stream = File.write(Std.string("$dirHaxeSrc/$className.hx"), false);
        ...
        stream.writeString("\n");
		stream.writeString(Std.string("// DECL: ${classInfo.declaration}\n"));
		stream.writeString(Std.string("class $className"));
		...
		if (Lambda.count(classInfo.parents) > 0)
		{
			...
			stream.writeString(Std.string(" extends $parent"));
		}
        ...
        for (method in classInfo.methods)
        {
            ...
            for (index in 0...params.length)
            {
                ...
                stream.writeString(Std.string("${param.name}:$type"));
                if (index < params.length - 1)
                    stream.writeString(", ");
            }
            ...
            stream.writeString(
                    if (method.type == mtConstructor)
                        ...
                        ConvertNativeToHaxeType(new ParamInfo(Std.string("${method.returnString} _r")))
                );
            stream.writeString("\n");
            stream.writeString("    {\n");
        ...
        trace(Std.string("INFO: Haxe source file\"${fileName}\" written."));
	}
    ...
    static function writeHaxeBody(stream:Output, classInfo:ClassInfo, methodInfo:MethodInfo)
    {
        ...
        if (methodInfo.type != mtConstructor)
        {
            ...
            stream.writeString(Std.string("$name"));
			//stream.writeString(Std.string("nativeInterface.$name"));
            //stream.writeString(Std.string("NativeInterface.$name"));
        ...
        else
        {
            ...
            stream.writeString(Std.string("new $name("));
